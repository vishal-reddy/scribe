...
export async function applyMigrations(db: D1Database): Promise<void> {
  await db.prepare(
    'CREATE TABLE IF NOT EXISTS documents (id text PRIMARY KEY NOT NULL, title text NOT NULL, content text NOT NULL, markdown text NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL, created_by text NOT NULL, last_edited_by text NOT NULL)'
  ).run();
